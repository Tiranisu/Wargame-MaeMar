...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($username) && !empty($password)) {
        ...
        try {
            ...
            if ($user) {
                if ($user['is_admin']) {
                    $error = "Les administrateurs ne peuvent pas se connecter ici.";
                } elseif ($password === base64_decode($user['password'])) {
...
<h1 class="w-1/2 mx-auto mt-5 text-center mb-2 text-4xl font-bold tracking-tight text-gray-900 dark:text-white">Connectez-vous</h1>
